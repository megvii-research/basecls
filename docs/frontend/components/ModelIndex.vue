...
const parallelOptions = {
  ...
  parallelAxis: [
    { dim: dataDimName.acc1.dim, name: dataDimName.acc1.display, type: 'value' },
    { dim: dataDimName.flops.dim, name: dataDimName.flops.display, type: 'log', logBase: 2 },
    { dim: dataDimName.params.dim, name: dataDimName.params.display, type: 'log', logBase: 2 },
    { dim: dataDimName.activations.dim, name: dataDimName.activations.display, type: 'log', logBase: 2 }
  ],
  ...commonOptions
}
    ...
    computed: {
      option: function () {
        ...
        if (this.chart_type === 'scatter' && this.scatterSeries.length !== 0) {
          ...
          options.yAxis = { ...options.yAxis, ...this.calculateBoundaryWithGap(options.series, 0, options.yAxis.type) }
          // axis formatter
